...
import { ChevronRight, ChevronDown, Copy, Check, Maximize2, Minimize2, AlertCircle, Braces } from 'lucide-react';

import { JSON_THEMES, type JsonThemeColors } from '../../../core/constants/jsonThemes';
...
const JsonNode: React.FC<JsonNodeProps> = ({
  ...
  const renderPrimitive = (val: unknown) => {
    if (val === null) {
      return (
        <span style={{ color: colors.nullValue }} className="font-semibold italic">
          null
        </span>
      );
    }
    if (val === undefined) {
      return <span className="text-slate-500 font-semibold italic">undefined</span>;
    }
    switch (typeof val) {
      case 'string':
    ...
    <div className="font-mono text-xs select-text">
      ...
      {isOpen && (
        <div className="pl-4 border-l border-[var(--border-color)] ml-2.5 my-0.5 space-y-0.5">
          {itemCount === 0 ? (
            <div className="text-[var(--text-subtle)] italic pl-1 py-0.5">empty</div>
          ) : (
...
export const JsonAdapter: React.FC<PreviewAdapterProps> = ({ tab, setHeaderActions }) => {
  const jsonTheme = useEditorStore((s) => s.settings.jsonTheme || 'default');
  const themeColors = JSON_THEMES[jsonTheme] || JSON_THEMES.default;

  const [expandMode, setExpandMode] = useState<{ isAllExpanded: boolean | null; key: number }>({
    isAllExpanded: null,
    key: 0,
  });
        ...
        >
          {copied ? (
            <>
              ...
              <span className="text-[var(--accent)] font-medium text-[10px]">Copied</span>
            </>
          ) : (
            <>
              <Copy className="w-3.5 h-3.5" />
              <span className="hidden md:inline text-[10px]">Copy JSON</span>
            </>
          )}
        </button>
      </div>
  ...
  }, [setHeaderActions, data, error, handleExpandAll, handleCollapseAll, handleCopyPretty, copied]);
  ...
  if (!rawContent.trim()) {
    return (
      <div className="h-full w-full flex flex-col items-center justify-center p-8 text-center text-[var(--text-muted)] select-none">
        ...
        <p className="text-xs font-medium text-[var(--text-main)]">Empty JSON Document</p>
